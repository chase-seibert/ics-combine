...
class IcsParsingTests(unittest.TestCase):
    ...
    def test_transform_event_can_skip_attendees(self):
        root = combine_ics.parse_ics(
            "\r\n".join(
                [
                    "BEGIN:VCALENDAR",
                    ...
                    "ATTENDEE;PARTSTAT=ACCEPTED:mailto:pat@example.com",
        ...
        self.assertNotIn("ATTENDEE;PARTSTAT=ACCEPTED", rendered)
        self.assertNotIn("ATTENDEE;PARTSTAT=NEEDS-ACTION", rendered)
        self.assertIn("ORGANIZER:mailto:lead@example.com", rendered)
        self.assertIn("Existing description\\n\\nSource calendar: Work Calendar", rendered)
...
class OutputSelectionTests(unittest.TestCase):
    ...
    def test_explicit_output_can_skip_event_attendees(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            output_file = pathlib.Path(tmpdir) / "bob.ics"
            config = {
                "calendars": [
            ...
            source = self.make_source("bob", "Bob")
            source.events[0].properties.extend(
                [
                    "ORGANIZER:mailto:lead@example.com",
                    "ATTENDEE;PARTSTAT=ACCEPTED:mailto:pat@example.com",
                    "ATTENDEE;PARTSTAT=NEEDS-ACTION:mailto:sam@example.com",
            ...
            text = output_file.read_text(encoding="utf-8")
            self.assertIn("UID:bob-event", text)
            self.assertIn("ORGANIZER:mailto:lead@example.com", text)
    ...
    def test_validate_config_rejects_non_boolean_skip_attendees(self):
        with self.assertRaises(combine_ics.ConfigError):
            combine_ics.validate_config(
                {
                    "calendars": [
